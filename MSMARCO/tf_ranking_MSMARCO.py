...
def get_train_inputs(features, labels, batch_size):
	...
	def _train_input_fn():
		...
		features_placeholder = {
			...
			#k: tf.sparse.placeholder(v.dtype, v.shape) for k, v in six.iteritems(features)
			#k: tf.placeholder(tf.string, (len(v), len(v[0]), v[0][0].shape[0]), name=k) for k, v in six.iteritems(features)
...
def train_and_eval():
	...
	features_test, labels_test = data.load_feature_from_data("TEST",
															input_size)
	...
	estimator = tf.estimator.Estimator(
			...
			config=tf.estimator.RunConfig(
					FLAGS.output_dir, save_checkpoints_steps=1))
...
def main(_):
	...
	if not os.path.exists(FLAGS.output_dir):
		...
		os.system('rm %s*' % FLAGS.output_dir)
